  ...
  <div>
    ...
    <el-form ref="formData"
      ...
      <el-form-item label="主图" prop="imageUrl">
        <!-- accept="image/png,image/jpeg" 接收文件上传类型
             action 指定上传地址，但是我们不使用它，但是这个函数必须要有
             show-file-list 是否显示已上传的文件列表，因为只传一张，所以不显示
      ...
      <el-form-item label="简介" prop="summary">
        ...
        placeholder="请输入简介" :autosize="{minRows: 3}" maxlength="2000"
        show-word-limit/>
      </el-form-item>
...
<script>
  export default  {
    ...
    methods: {
      ...
      async submitData() {     //调用新增或者更新文章接口
        //判断formData的id是否为空，如果为空是新增，不为空则为修改
        if(this.formData.id) {  //修改
          const {code, data} = await  this.$updateArticle(this.formData);
          if(code === 20000) {  //文章修改成功，跳转到文章详情界面
            this.$message.success("文章修改成功！");
            await this.$router.push(`/article/${data}`);
          }else {  //文章修改失败
            this.$message.error("文章修改失败，请重试！");
          }
        }else {  //新增
          const {code, data} = await this.$addArticle(this.formData);
          if(code === 20000) {  //新增文章成功
    ...
    async asyncData({app, query}) {
      ...
      if(query.id) {  //如果参数文章Id不为空，那么调用后台获取文章数据
